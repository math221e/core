...
class TroubleshootTransferOwnership extends Command {
	...
	public function __construct(IDBConnection $connection, IManager $shareManager, ProviderFactory $shareProviderFactory) {
	 ...
	 * @param boolean $fix
	 */
	private function findInvalidShareOwner(InputInterface $input, OutputInterface $output, $fix) {
		$output->writeln([
			"<info>==========================</>",
	 ...
	 * @param boolean $fix
	 */
	private function findInvalidReshareInitiator(InputInterface $input, OutputInterface $output, $fix) {
		$output->writeln([
			"<info>==========================</>",
	...
	private function getResharesForUser($userId) {
		$query = $this->connection->getQueryBuilder();
	...
	private function adjustShareInitiator($shareId, $newUidInitiator) {
		$query = $this->connection->getQueryBuilder();
	...
	private function adjustShareOwner($shareId, $newUidOwner) {
		$query = $this->connection->getQueryBuilder();
	...
	private function deleteCorruptedShare($shareData) {
